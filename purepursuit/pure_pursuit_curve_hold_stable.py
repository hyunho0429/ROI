...
def apply_curve_sign_hysteresis(
    ...
    """
    한 방향 장커브에서 작은 반대 방향 보정 때문에
    조향 부호가 반복적으로 뒤집히는 현상을 억제한다.
    """
    ...
    # 커브 방향과 반대되는 작은 조향은 0으로 억제
    if steer_target * expected_sign < 0.0:
        if abs(steer_target) < CURVE_SIGN_HYSTERESIS:
            return 0.0

        # 이전 조향이 올바른 방향이면 급격한 반전 대신 유지
        if previous_steer * expected_sign > 0.0:
...
def main():
    ...
    try:
        while True:
            ...
            average_curvature = calculate_average_curvature(
                path,
                nearest_index
            )
            ...
            raw_steer = (
                STEER_FEEDFORWARD_GAIN * feedforward_steer
                + STEER_FEEDBACK_GAIN * pp_steer
            )

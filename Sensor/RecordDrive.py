...
def push_status_snapshot(snapshot, t_sim):
    """Store one status sample for :func:`interp_status`.

    LiveLaneOracle normally receives EgoVehicleStatus UDP directly.  The
    integrated ROS stack already owns that UDP port, so its ROS adapter uses
    this function to feed the exact same interpolation history without opening
    a second socket.
    """
    ...
    with _status_lock:
        _latest_status = sample
        if not _status_hist or timestamp > _status_hist[-1][0]:
            _status_hist.append((timestamp, sample))
...
def status_worker(ip, port):
    ...
    while True:
        try:
            ...
            push_status_snapshot(snapshot, t_sim)

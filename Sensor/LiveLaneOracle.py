...
이 노드는 저장소의 MORAI MGeo가 현재 시나리오 지도와 일치할 때 사용한다. 다른
지도나 실차처럼 같은 HD MAP을 사용할 수 없는 환경에서는 끄고, 학습된
PolyRegression 모델이 같은 출력 형식을 채우도록 교체할 수 있다.
...
def build_arg_parser():
    ...
    ap.add_argument(
        ...
        type=float,
        default=-1.361,
        help="HD MAP 투영 시 자차 yaw에 더할 보정값 [deg] (기본: -1.361)",
    )
...
def _configure_projection(yaw_offset_deg):
    """Apply the launch-time yaw correction used by GenerateLabels.to_ego()."""
    GenerateLabels.YAW_OFFSET_DEG = float(yaw_offset_deg)


def run(args):
    # GenerateLabels.to_ego()가 참조하는 모듈 전역값에 보정값을 주입한다.
    # EgoVehicleStatus.heading은 degree이므로 이 값도 degree 단위다.
    _configure_projection(args.yaw_offset_deg)
    ...
    if args.ros_status_topic:
        ...
        rospy.logwarn(
            "HD-map lane oracle: status=%s dashed_output=%s yaw_offset=%+.3f deg",
            args.ros_status_topic,
            args.dashed_lane_topic,
            GenerateLabels.YAW_OFFSET_DEG,
        )

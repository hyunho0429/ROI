...
3열 경로처럼 속도 열이 없으면 `--target-speed-kmh`(기본 10 km/h)를 사용한다.
...
차량이 차선 안에서 한쪽으로 일정하게 치우치면 `--path-lateral-offset`으로
기준 경로를 좌우 평행 이동한다. 양수는 경로 진행 방향의 왼쪽, 음수는 오른쪽이다.
오른쪽 바퀴가 오른쪽 차선에 붙는 상황에서는 `0.8~1.0 m`부터 시험한다.

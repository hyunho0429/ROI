...
from path_planning.lidar_obstacle_tracker import ObstacleTracker
from path_planning.lidar_behavior_decision import BehaviorDecision
...
try:
    ...
    LIDAR_PORT = 2001
try:
    from path_planning.morai_competition_config import LIDAR_POSE_IP, LIDAR_POSE_PORT
except ImportError:
...
from path_planning.morai_udp_gps import GpsPacketError
from path_planning.morai_udp_imu import ImuPacketError
...
def main():
    ...
    params = {
        ...
        "behavior_topic": _param(
            "behavior_topic",
            "/morai/lidar/behavior_decision",
        ),
        ...
        # Behavior decision (judgment only, no vehicle control)
        "behavior_enabled": _bool_param("behavior_enabled", True),
        "behavior_front_half_width_m": float(
            _param("behavior_front_half_width_m", 1.25)
        ),
        "behavior_monitor_ttc_s": float(_param("behavior_monitor_ttc_s", 6.0)),
        "behavior_slowdown_ttc_s": float(
            _param("behavior_slowdown_ttc_s", 4.5)
        ),
        "behavior_avoid_ttc_s": float(_param("behavior_avoid_ttc_s", 3.0)),
        "behavior_brake_ttc_s": float(_param("behavior_brake_ttc_s", 1.5)),
        "behavior_follow_distance_m": float(
            _param("behavior_follow_distance_m", 35.0)
        ),
        "behavior_emergency_distance_m": float(
            _param("behavior_emergency_distance_m", 8.0)
        ),
        "behavior_side_front_clearance_m": float(
            _param("behavior_side_front_clearance_m", 20.0)
        ),
        "behavior_side_rear_clearance_m": float(
            _param("behavior_side_rear_clearance_m", 10.0)
        ),
        "behavior_side_half_width_m": float(
            _param("behavior_side_half_width_m", 1.35)
        ),
        "behavior_reaction_time_s": float(
            _param("behavior_reaction_time_s", 0.6)
        ),
        "behavior_comfortable_decel_mps2": float(
            _param("behavior_comfortable_decel_mps2", 3.5)
        ),
        "behavior_emergency_decel_mps2": float(
            _param("behavior_emergency_decel_mps2", 7.0)
        ),
        "behavior_stopping_margin_m": float(
            _param("behavior_stopping_margin_m", 3.0)
        ),
        # Until map/lane-line data is connected, these can be set manually.
        # Set False where a solid line makes that lane change illegal/penalized.
        "left_lane_change_allowed": _bool_param(
            "left_lane_change_allowed", True
        ),
        "right_lane_change_allowed": _bool_param(
            "right_lane_change_allowed", True
        ),
        "tracker_match_distance_m": float(
            _param("tracker_match_distance_m", 3.0)
        ),
        "tracker_max_age_s": float(_param("tracker_max_age_s", 0.8)),
    ...
    if params["deskew_pose_source"] not in ("sensor_udp", "fused_pose_udp"):
        raise ValueError(
            "deskew_pose_source must be 'sensor_udp' or 'fused_pose_udp'"
        )
    ...
    behavior_publisher = rospy.Publisher(
        params["behavior_topic"],
        String,
        queue_size=1,
    )
    ...
    if params["deskew_enabled"]:
        try:
            if params["deskew_pose_source"] == "fused_pose_udp":
    ...
    if not params["deskew_enabled"]:
        ...
        pose_input_description = (
            "fused_pose_udp=%s:%d"
            % (params["pose_bind_ip"], params["pose_port"])
            if params["deskew_pose_source"] == "fused_pose_udp"
            else "sensor_udp gps=%s:%d imu=%s:%d"
            % (
                params["bind_ip"],
                params["gps_port"],
                params["bind_ip"],
    ...
    obstacle_tracker = ObstacleTracker(
        match_distance_m=params["tracker_match_distance_m"],
        max_track_age_s=params["tracker_max_age_s"],
    )
    behavior_decider = BehaviorDecision(
        ...
        side_front_clearance_m=params["behavior_side_front_clearance_m"],
        side_rear_clearance_m=params["behavior_side_rear_clearance_m"],
        lane_width_m=params["adjacent_lane_width_m"],
        side_half_width_m=params["behavior_side_half_width_m"],
        reaction_time_s=params["behavior_reaction_time_s"],
        comfortable_decel_mps2=params["behavior_comfortable_decel_mps2"],
        emergency_decel_mps2=params["behavior_emergency_decel_mps2"],
        stopping_margin_m=params["behavior_stopping_margin_m"],
    )
        ...
        while not rospy.is_shutdown():
            ...
            if wrapped and packet_batches:
                ...
                if params["cluster_enabled"]:
                    ...
                    tracked_obstacles = obstacle_tracker.update(
                        clusters,
                        now,
                    )
                ...
                if params["behavior_enabled"]:
                    behavior_result = behavior_decider.decide(
                        tracked_obstacles,
                        left_lane_change_allowed=params[
                            "left_lane_change_allowed"
                        ],
                        right_lane_change_allowed=params[
                            "right_lane_change_allowed"
                        ],
                    )
                    behavior_publisher.publish(
                        String(
                            data=json.dumps(
                                behavior_result,
                                ensure_ascii=False,
                    ...
                    front_info = behavior_result.get("front_obstacle")
                    if front_info is None:
                        rospy.loginfo_throttle(
                            0.5,
                            "BEHAVIOR=%s | reason=%s | left_safe=%s right_safe=%s",
                            behavior_result["decision"],
                            behavior_result["reason"],
                            behavior_result["left_safe"],
                            behavior_result["right_safe"],
                        )
                    else:
                        ttc_value = front_info.get("ttc_s")
                        ttc_text = (
                            "inf"
                            if ttc_value is None
                            else "{:.2f}s".format(ttc_value)
                        )
                        rospy.loginfo_throttle(
                            0.5,
                            "BEHAVIOR=%s | front=%.1fm closing=%.1fm/s TTC=%s "
                            "| stop_req=%.1fm emergency_stop_req=%.1fm stop_feasible=%s "
                            "| left_safe=%s right_safe=%s | reason=%s",
                            behavior_result["decision"],
                            front_info["distance_m"],
                            front_info["closing_speed_mps"],
                            ttc_text,
                            front_info["comfortable_stop_distance_m"],
                            front_info["emergency_stop_distance_m"],
                            behavior_result["stop_feasible"],
                            behavior_result["left_safe"],
                            behavior_result["right_safe"],
                            behavior_result["reason"],
                        )

...
class KCityDijkstraPathPublisher:
    def __init__(self):
        ...
        # 차량 실시간 상태 토픽 수신 등록
        rospy.Subscriber(self.ego_topic, EgoVehicleStatus, self._ego_callback)
    ...
    def _ego_callback(self, msg):
        # 최초로 데이터가 수신되었을 때 터미널에 기분 좋은 초록색 완료 로그를 한 번 띄워줍니다.
        if self.last_odom_xy is None:
            rospy.loginfo("⚡ [SUCCESS] /Ego_topic data stream connected! Latency resolved.")
    ...
    def _resolve_start_node(self):
        ...
        if self.last_odom_xy is not None:
            ...
            rospy.loginfo(f"DYNAMIC START -> Node: {start_node} ({distance:.2f}m from Ego)")
            return start_node

        # 에러를 던져서 멈추게 하지 않고, 데이터 수신 대기 상태임을 명확히 경고 로그로 표현
        raise rospy.ROSException(f"Vehicle position data on [{self.ego_topic}] has not been received yet. Please check if simulator is playing.")

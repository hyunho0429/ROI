...
"""`real_lane.py` 를 돌려서 제어가 쓸 값을 내보낸다.
    ...
    ROS 노드로:     python3 real_lane_node.py
...
어댑터가 차선 의미를 만들 때 읽는 핵심 키는 다음과 같다.

    left_lane.detected   left_lane.type   left_lane.dashed
    stopline_detected    stopline_distance_m        (right_lane 도 같은 셋)

이 노드는 그 키와 기존 제어가 읽는 최상위 `confidence`, `center_source`를 함께
내보낸다. `lane_info_runner.py`가 이 파일을 실행하고, 소비자는 새 상태인
`from_guide`, `coasted`를 확인한 뒤 차선 변경 허용 여부를 판단한다.
...
def build_payload(res, opt, timing):
    ...
    lane_confidences = [float(c.confidence) for c in (left, right) if c is not None]
    confidence = min(lane_confidences) if lane_confidences else 0.0
    center_source = ("both" if (left is not None and right is not None)
    ...
    out = {
        ...
        "lane_state": center_source,
        # Existing highway control consumers use these top-level v2 fields.
        # Keep them while also publishing each boundary's richer confidence.

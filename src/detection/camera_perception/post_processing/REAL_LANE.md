...
```bash
# ROS 노드로
python3 $(rospack find camera_perception)/post_processing/real_lane_node.py
...
target = pkg_root / "post_processing" / "real_lane_node.py"     # 이걸로
```

- 토픽 이름과 포트는 같습니다.
- 이 저장소의 `lane_info_runner.py`는 이미 위 경로를 사용합니다.
- 기존 고속도로 제어와의 호환성을 위해 최상위 `confidence`, `center_source`도
  함께 발행하며, 어댑터와 회피 로직은 `from_guide`, `coasted`를 확인합니다.

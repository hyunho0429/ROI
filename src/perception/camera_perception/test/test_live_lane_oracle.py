...
class LiveLaneOracleTest(unittest.TestCase):
    def test_default_yaw_offset_is_minus_1_361_degrees(self):
        args = LiveLaneOracle.build_arg_parser().parse_args(
            ["--mgeo", "map", "--cam-set", "camera.json"]
        )
        self.assertAlmostEqual(args.yaw_offset_deg, -1.361)

    def test_yaw_offset_is_applied_to_generate_labels_projection(self):
        original = GenerateLabels.YAW_OFFSET_DEG
        try:
            LiveLaneOracle._configure_projection(-1.361)
            self.assertAlmostEqual(GenerateLabels.YAW_OFFSET_DEG, -1.361)
        finally:
            GenerateLabels.YAW_OFFSET_DEG = original

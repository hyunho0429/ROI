#!/usr/bin/env python3
"""Camera-lane-aware maneuver supervisor for Frenet obstacle avoidance.
...
* reject a bypass that visibly leaves the current lane without going through the
  lane-change permission/gap checks;
...
class LaneManeuverSupervisor:
    def __init__(self) -> None:
        rospy.init_node("lane_maneuver_supervisor", anonymous=False)
        ...
        self.prefer_lane_change = bool(rospy.get_param("~prefer_lane_change", True))
        self.enforce_camera_lane_bounds_on_bypass = bool(
            rospy.get_param("~enforce_camera_lane_bounds_on_bypass", True)
        )
        ...
        self.fallback_bypass_max_abs_d_m = float(
            rospy.get_param("~fallback_bypass_max_abs_d_m", 0.75)
        )
        ...
        rospy.logwarn(
            "Lane maneuver supervisor started: camera lane_info=%s, MGeo lane-change execution disabled upstream, "
            "lane-change avoidance uses dashed boundary + dynamic gap + OBB/steering checks",
    ...
    def _generate_lane_change_candidate(
        ...
        local_end_s = min(ego_s + self.local_length_m, self.reference.total_length_m)
        obstacle_front_s = threat.s - threat.longitudinal_half_m
        obstacle_rear_s = threat.s + threat.longitudinal_half_m
        hold_start_s = obstacle_front_s - self.lane_change_longitudinal_margin_m
        hold_end_s = obstacle_rear_s + self.lane_change_longitudinal_margin_m
        return_end_s = hold_end_s + self.lane_change_return_length_m
        if hold_end_s <= ego_s or return_end_s > local_end_s:
            return None

        transition_start_s = max(ego_s, hold_start_s - max(0.5, departure_length_m))
        if hold_start_s > transition_start_s + 1.0e-6:
            transition_end_s = hold_start_s
        else:
            remaining = max(0.0, hold_end_s - transition_start_s)
            if remaining < 0.2:
                return None
            transition_end_s = min(
                hold_end_s,
                transition_start_s + max(self.lane_change_min_transition_m, 0.5 * remaining),
            )
        transition_len = transition_end_s - transition_start_s
        ...
        while s <= local_end_s + 1.0e-6:
            if s < transition_start_s:
                d = ego_d
            elif s <= transition_end_s and transition_len > 1.0e-6:
                u = (s - transition_start_s) / transition_len
                d = _quintic_lateral_transition(
                    ego_d, target_d, ego_d_slope, transition_len, u
                )
            elif s <= hold_end_s:
                d = target_d
            elif s <= return_end_s:
                ...
                d = 0.0
            _append_unique(path, self.reference.frenet_to_map(s, d))
            s += spacing
        ...
        for exact_s, exact_d in (
            ...
            if ego_s <= exact_s <= local_end_s:
                _append_unique(path, self.reference.frenet_to_map(exact_s, exact_d))
    ...
    def _base_bypass_road_legal(
        ...
        if not self.enforce_camera_lane_bounds_on_bypass:
            return True, "camera_bounds_disabled"
    ...
    def _timer_cb(self, _event) -> None:
        ...
        if lane_info is not None and current_center_d is not None and threat is not None:
            width = float(lane_info.get("lane_width_m"))
            ...
            for side, sign in (("left", 1.0), ("right", -1.0)):
                ...
                if not allowed:
                    ...
                    continue
                target_d = current_center_d + sign * width
        ...
        if base_safe and base_kind == "bypass" and self.base_path is not None:
            bypass_ok, bypass_reason = self._base_bypass_road_legal(
                self.base_path, lane_info, current_center_d
            )
